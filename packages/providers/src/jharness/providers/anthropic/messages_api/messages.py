...
from jharness.providers.anthropic.errors import ANTHROPIC_JSON, AnthropicError
...
def encode_message(
    ...
    if role == "assistant" and message.tool_calls:
        ...
        from jharness.providers.anthropic.messages_api.tools import encode_assistant_tool_uses
...
def encode_user_content_part(part: ContentPart, profile: AnthropicProfile) -> JsonObject:
    provider_block = _provider_block(part, role="user", profile=profile)
...
def _encode_system_parts(
    ...
    for part in parts:
        provider_block = _provider_block(part, role="system", profile=profile)
        if provider_block is not None:
...
def _encode_assistant_part(part: ContentPart, profile: AnthropicProfile) -> JsonObject:
    provider_block = _provider_block(part, role="assistant", profile=profile)
    if provider_block is not None:
        return provider_block
    if part.type == "text":
        return {"type": "text", "text": part.text or ""}
...
def _provider_block(
    ...
    *,
    ...
    if block_type == "document" and not profile.supports_file_input:
        raise AnthropicError(f"{profile.name} does not support file input")
    _validate_provider_block(mapping, block_type)
    return dict(mapping)


def _validate_provider_block(block: Mapping[str, Any], block_type: str) -> None:
    if block_type == "text":
