...
from jharness.providers.openai.errors import OPENAI_JSON, OpenAIChatCompletionsError
...
def encode_chat_message(
    ...
    if role == "assistant" and message.tool_calls:
        from jharness.providers.openai.chat_completions.tools import encode_assistant_tool_calls
...
def _encode_assistant_content(
    ...
    for part in parts:
        ...
        provider_block = _provider_block(part)
        if provider_block is None:
            ...
            provider_block = {"type": "refusal", "refusal": refusal}
        blocks.append(provider_block)
    return blocks


def _provider_block(part: ContentPart) -> JsonObject | None:
