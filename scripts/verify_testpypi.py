"""Install a coordinated release from TestPyPI and run public smoke examples."""
...
_VERSION = re.compile(r"\d+\.\d+\.\d+(?:(?:a|b|rc|dev)\d+)?")
_PROJECT = """\
[project]
...
def project_document(version: str) -> str:
    """Return an isolated uv project that pins only JHarness to TestPyPI."""
...
def main() -> int:
    """Retry index propagation, then execute both public smoke examples."""

    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("version")
    parser.add_argument("--attempts", type=int, default=12)
    parser.add_argument("--delay", type=float, default=15)
    parser.add_argument("--examples", type=Path, default=Path("examples/python"))
    ...
    with tempfile.TemporaryDirectory(prefix="jharness-testpypi-") as temporary:
        ...
        (project / "pyproject.toml").write_text(document)
