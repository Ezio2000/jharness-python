...
def run_example(name: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, str(ROOT / "examples" / "python" / name)],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
...
def test_ci_references_existing_sources_and_required_commands() -> None:
    ...
    assert '--package "$package"' in workflow
    assert "benchmarks/runtime_smoke.py" in workflow
    assert "scripts/verify_wheels.py" in workflow
    assert ".github/scripts/verify_wheels.py" not in workflow
